...
function ttnyc_v2_preprocess_html(&$vars) {
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = ttnyc_v2_id_safe('page-'. $path);
    $vars['classes_array'][] = ttnyc_v2_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('ttnyc_v2_ie_enabled')) {
    $ttnyc_v2_ie_enabled_versions = theme_get_setting('ttnyc_v2_ie_enabled_versions');
    if (in_array('ie8', $ttnyc_v2_ie_enabled_versions, TRUE)) {
...
function ttnyc_v2_preprocess_page(&$vars, $hook) {
...
function ttnyc_v2_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from ttnyc_v2_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function ttnyc_v2_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function ttnyc_v2_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('ttnyc_v2_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('ttnyc_v2_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('ttnyc_v2_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('ttnyc_v2_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('ttnyc_v2_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function ttnyc_v2_id_safe($string) {
 ...
 *
 */
function ttnyc_v2_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = ttnyc_v2_id_safe($element['#title']);
 ...
 */
function ttnyc_v2_preprocess_menu_local_task(&$variables) {
 ...
 */
function ttnyc_v2_menu_local_tasks(&$variables) {
  $output = '';
